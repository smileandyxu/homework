	...
	case BUTTON_DOWN:
		...
		else
		{
			...
			/*else if (menu_check(nx, ny)) {
				mode = (menu_check(nx, ny) % 5), current_id = 0;
				draw(0), draw(1);
			}
			else {
				mode = 0, current_id = 0;
...
bool onLayerEdge(int id, double x, double y) { /**/
	if (layer[id].kind == 1) {
		...
		return (fabs((y2 - y1) * x - (x2 - x1) * y - x1 * y2 + x2 * y1) < range);
	}
		...
		return (inRange(layer[id].x1, x) || inRange(layer[id].x2, x)) && (inRange(layer[id].y1, y) || inRange(layer[id].y2, y));
	}
	else if (layer[id].kind == 3) {
		double midy = (layer[id].y1 + layer[id].y2) / 2;
		double rr = max(fabs(layer[id].x1 - layer[id].x2), fabs(layer[id].y1 - layer[id].y2));
		return doubleEqual(dist(x, y, layer[id].x1, midy) + dist(x, y, layer[id].x2, midy), rr);
	}
}
