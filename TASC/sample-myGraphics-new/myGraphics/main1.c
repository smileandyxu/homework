...
void Main()
{
    ...
    DrawHouse(cx - HouseWidth / 2.0, cy - (HouseHeight + AtticHeight) / 2.0);

    MovePen(cx - 0.1, cy + (HouseHeight - AtticHeight) / 2.0 + 0.05);
    DrawTextString("ÄãºÃ£¡");
}
...
{
    DrawOutline(x, y);
    DrawDoor(x + (HouseWidth - DoorWidth) / 2.0, y);
    DrawWindows(x, y);
}
...
void DrawDoor(double x, double y)
{
    ...
    DrawCenteredCircle(x + DoorWidth - DoorknobInset,
                       y + DoorHeight / 2.0, DoorknobRadius);
}
